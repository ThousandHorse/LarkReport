...
GitHub Actions：
  毎朝 7:00 になる → 自動でプログラムが動く ← これが LarkReport
```

GitHub というコードを管理するサービスの中に、「スケジュール機能」が無料で使えます。
設定ファイル（`.yml` という形式）に「毎日 7:00 に動かして」と書くだけで OK です。
...
'0 22 * * *'
 ...
 │  │  │ └──── 月（* = 毎月）
 │  │  └────── 日（* = 毎日）
 │  └────────── 時（UTC で 22 時 = 日本時間の朝 7 時）
 └──────────── 分（0 分ちょうど）

日本時間（JST）は UTC より 9 時間進んでいる
朝 7:00 JST = 22:00 UTC（前日）
夜 22:00 JST = 13:00 UTC
```
