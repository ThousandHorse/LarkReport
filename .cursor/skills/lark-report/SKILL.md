...
### ファイル先頭コメント（必須）
各モジュールのファイル先頭に、以下の内容を JSDoc ブロックコメントで記載すること。

```javascript
/**
 * モジュール名.js
 *
 * このモジュールの処理概要を1〜3文で記載する。
 *
 * 戻り値（関数の場合）:
 *   { フィールド名: 型 }
 *
 * 注意事項（環境依存・スコープ制約など、ある場合のみ記載）:
 */
```

### 関数コメント
export する関数には `@param` と処理の意図を JSDoc で記載すること。
...
### インラインコメント
WHY が自明でない箇所（環境依存・ワークアラウンド・非自明な挙動）にのみ付ける。
WHAT を説明するコメントは不要（識別子で十分）。
